...
for (f in files) {
	...
	for (c in unique(obj@meta.data[,cluster_column]) ) {
		...
		write.table( out, file=paste( "SubCluster_DE_Tables", paste( tag, c, "combined_DE_table.csv", sep="_"), sep="/"), sep=",", 
			row.names=T, col.names=T, quote=T )
	}
	...
	write.table( out, file=paste( "SubCluster_DE_Tables", paste( tag, c, "combined_DE_TransReject.csv", sep="_"), sep="/"), sep=",", 
			row.names=T, col.names=T, quote=T )
	...
	write.table( out, file=paste( "SubCluster_DE_Tables", paste( tag, c, "combined_DE_Sex.csv", sep="_"), sep="/"), sep=",", 
			row.names=T, col.names=T, quote=T )

	out_3pr <- FindMarkers(dat_3pr, group.by="donor_age_group", ident.1="young", ident.2="elderly", logfc.threshold=0)
	
	write.table( out_3pr[,-1], file=paste( "SubCluster_DE_Tables", paste( tag, c, "3pr_DE_Age.csv", sep="_"), sep="/"), sep=",", 
			row.names=T, col.names=T, quote=T )
