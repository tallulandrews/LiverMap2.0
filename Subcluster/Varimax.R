...
#	cell_cluster_col <- cluster_col_palette[obj@meta.data[,cluster_column]]
	...
	for (i in 1:ncol(cell_loadings)) {
		...
		all_scores <- rbind(all_scores, c(res$coefficients[,1], res3$coefficients[,1], res2$coefficients[,1])) # before 4,4,1
	...
	pdf(paste(tag, "associations_coeff_varimax.pdf", sep="_"), width=9, height=6.5)
	# Sex
