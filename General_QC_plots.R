...
perc <- table(seur_obj@meta.data$sample, seur_obj@meta.data[,cluster_col])
...
perc <- table(seur_obj@meta.data$Phase, seur_obj@meta.data[, cluster_col])
perc <- t(t(perc)/colSums(perc))
...
perc <- table(map_cell_types(seur_obj@meta.data$consistent_labs), seur_obj@meta.data[,cluster_col])
perc <- t(t(perc)/colSums(perc))
...
perc <- table(map_cell_types(seur_obj@meta.data$marker_labs), seur_obj@meta.data[,cluster_col])
perc <- t(t(perc)/colSums(perc))
type_cols <- Cell_type_colours[match(rownames(perc),Cell_type_colours[,1]),2]
